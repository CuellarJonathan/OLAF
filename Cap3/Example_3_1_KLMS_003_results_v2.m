...
gamma_c = 0.7; % Limiar para adicionar um novo ponto ao dicionário
I_max = 50; % Tamanho máximo do dicionário
...
xl1=zeros(K,1);
xl2=xl1; 
xl1(2:K)=v(1:K-1);    % x(k-1)
xl2(3:K)=v(1:K-2);    % x(k-2)
...
kernel_param = [1 1 0.01]; % Parâmetro do kernel (pode variar dependendo do tipo de kernel)
...
% Inicialização
x = ones(length(d), I_max); % Dicionário
e = zeros(1, length(d)); % Erro
l_0 = zeros(1, length(d)); % Índices iniciais dos pontos no dicionário
...
%         size(x(k:-1:1))
        ...
        if k <= N+1
            ...
            for l = 1:l_max
                kernel_values = calculate_kernel_values(x(k-l_max:-1:k-l_max-N,1),x(k:-1:k-N,1), kernel_type, kernel_param);
                kernel_lib_temp(l) = kernel_values;
                g(k-1) = 2*mu*e(l)*kernel_lib_temp(l) + 2*mu*calculate_kernel_values(x(k:-1:k-N,1),x(k:-1:k-N,1), kernel_type, kernel_param);
            end
        end
...
%         g(k) = 2 * mu * sum(repmat(e(1:k-1), 1, size(kernel_values, 1)) .* kernel_values, 1) + 2 * mu * sum(kernel_values);
        ...
        if max_kernel_value <= gamma_c
            ...
            if I <= I_max
                ...
                x(:, I_max) = d(k) * ones(length(d), 1); % Substitui x(l_max) por x(k) no dicionário
            end
        else
